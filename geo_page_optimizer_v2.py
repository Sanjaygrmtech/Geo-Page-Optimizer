...
def audit_citability(soup, url, profile, page_intent, gemini_key=None):
    results = {"checks": [], "score": 0, "max_score": 20, "fixes": [], "gemini_analysis": None}
    intent_config = PAGE_INTENTS.get(page_intent, {})
    ...
    # Gemini analysis
    if gemini_key and page_intent == "informational":
        try:
            import google.generativeai as genai
            genai.configure(api_key=gemini_key)
            model = genai.GenerativeModel("gemini-2.0-flash")
            industry_ctx = f"Industry: {profile.get('industry', 'general')}. Site: {profile.get('name', '')}." if profile else ""
            prompt = f"""Analyze for AI citability. {industry_ctx}\nURL: {url}\nContent (first 3000 chars):\n{text[:3000]}\n\nScore 1-5 each with ONE specific fix:\n1. DEFINITIONAL_CLARITY: Clear, quotable definitions?\n2. UNIQUE_DATA: Original statistics or case study data?\n3. AUTHORITY_SIGNALS: Author credentials, source citations, expert quotes?\n4. STRUCTURED_ANSWERS: Directly answers common questions?\n5. FRESHNESS: Date indicators showing recent updates?\n\nRespond ONLY in JSON, no markdown:\n{{"definitional_clarity":{{"score":N,"fix":"action"}},"unique_data":{{"score":N,"fix":"action"}},"authority_signals":{{"score":N,"fix":"action"}},"structured_answers":{{"score":N,"fix":"action"}},"freshness":{{"score":N,"fix":"action"}}}}"""
            response = model.generate_content(prompt)
            resp_text = response.text.strip().replace("```json", "").replace("```", "").strip()
            results["gemini_analysis"] = json.loads(resp_text)
        except Exception as e:
...
def main():
    ...
    if run_button and url:
        ...
        cite_results = audit_citability(soup, url, profile, page_intent, gemini_key if gemini_key else None)
        ...
        if cite_results.get("gemini_analysis") and "error" not in cite_results.get("gemini_analysis", {}):
            st.markdown("**🤖 Gemini Analysis:**")
            for dim, data in cite_results["gemini_analysis"].items():
